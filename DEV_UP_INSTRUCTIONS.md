...
**Repository:** `GlacierEQ/motherduck-local-cloud-query-passport`  
**Company lens (independent):** MotherDuck (`motherduck`)  
**Innovation:** Local-Cloud Query Passport  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim MotherDuck affiliation, proprietary access, or production deployment.
...
Stamp queries with residency class and result digests; refuse cross-boundary plans without passport.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply MotherDuck employment, endorsement, or proprietary systems access.
...
File: `src/local_cloud_query_passport.py`

- Expand `LocalCloudQueryPassport` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_local_cloud_query_passport.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_local_cloud_query_passport.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary MotherDuck APIs
