"""Local-Cloud Query Passport — SCAFFOLD STUB.

Company lens: MotherDuck (independent; no affiliation).
Bottleneck: Hybrid local/cloud SQL needs an explicit data-residency and result-provenance passport.
...
class LocalCloudQueryPassportRequest:
...
@dataclass(frozen=True)
class LocalCloudQueryPassportReceipt:
    decision: Decision
    reasons: tuple[str, ...]
    digest: str
...
class LocalCloudQueryPassport:
    """Central mechanism stub.

    Contract the filling AI must preserve:
    - `evaluate(req)` returns a LocalCloudQueryPassportReceipt
    ...
    def evaluate(self, req: LocalCloudQueryPassportRequest) -> LocalCloudQueryPassportReceipt:
        reasons: list[str] = []
        ...
        if reasons:
            ...
            return LocalCloudQueryPassportReceipt(
...
Mechanism = LocalCloudQueryPassport
